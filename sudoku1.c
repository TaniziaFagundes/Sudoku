...
{ 
    ...
    int contadorc =0;
    system("color 3f");
    printf("  |1|2|3| |4|5|6| |7|8|9|\n\n"); //linha para indicar coluna da matriz
     while(linha < N)   
     {
...
void preenchercomzero(int matriz[N][N],int matrizauxiliar[N][N]) //toda a matriz recebe valor 0
{
    int coluna=0;
    int linha=0;
     while(linha < N)
...
int verificarcoluna(int valorusuario,int coluna ,int matriz[N][N])  //funcao (1.1) verifica se ja existe aquele valor da coluna !!
{
  int cont= 0;
  int verifica = 1;    //o valor dado pelo usuario e valido !!
  
  while((cont < 9) && (verifica == 1))  /*a variavel coluna Ã© fixa, para que ela pecorra a coluna escolhida do usuario*/
  {
  ...
  return verifica; // ira retorna para a funcao main ,no caso sera um boolean (true or false)
}

int verificarlinha(int valorusuario,int linha ,int matriz[N][N])  //funcao (1.2) que verifica se ja existe valor escolhido na linha
{
  int cont= 0;
  int verifica = 1;                                          
  ...
  return verifica;  //se o retorno for 0(false), e porque ja existe aquele valor na linha
}

int verificabloco(int valorusuario,int linha,int coluna,int matriz[N][N]) //funcao (1.3) verifica se ja existe valor nos blocos 
{
    int verifica = 1;
...
void randomico(int matriz[N][N],int matrizauxiliar[N][N]) //funÃ§ao (1.5)insere 8  valores aleatorios de 1 a 9 na matriz. //A matriz auxiliar serve para deixar os valores randomicos imutavel
{ 
  int cont = 0;
  int linharand ;
  int colunarand ; 
 ...
 while(cont < 8)
 {
   srand(time(NULL));  //iniciando o randomico,sem essa funÃ§ao ele dÃ¡ valores iguais a todo tempo
   linharand = rand()%9;
   colunarand = rand()%9;
        if(matriz[linharand][colunarand]==0) //esse if estÃ¡ sem o else (se der valor falso !=0 ele ira ficar executando em loop infinito)
        {
          ...
          while((verificarlinha(valor_gerado,linharand,matriz) == 0)|| //basta que 1 desses seja =1 para que ele gere outto valor
                ...
                (verificabloco(valor_gerado,linharand,colunarand,matriz)==0)){ //essas duas linhas chama as 3 funcoes de verificaÃ§Ã¢o para que os randomicos entre na regra do jogo
                 srand(time(NULL));
...
int verificaposicao (int linha,int coluna,int matriz[N][N]) //funÃ§Ã£o (1.4) que verifica se local escolhido pelo usuario estÃ¡ vazio .
{                                                            
  int verifica = 1;
...
int tamanhovalor (int valorusuario) // funÃ§ao (1.6) se estÃ¡ funÃ§Ã£o der false na funÃ§ao main, ela deve exibir uma mensagem de valor incompativel
{
  int verifica = 1;

  if((valorusuario < 0) || (valorusuario > 9)) // o valorusuario vai poder ser 0 ,para o caso do usuario querer mudar o valor jÃ¡ iserido,ele poderÃ¡ retornar a 0 para recolocar outro valor valido depois
  {
    verifica = 0;
  }
  return verifica;
...
void main()
{
  ...
  int opcao = 0;
  int opcao1 = 0;
  int ganhador = 8; //começa com 8 por conta dos primeiros 8 randomicos
  // 1 = true , 0 = false
  ...
  {  
     ...
     scanf("%d",&valorusuario);
      if((verificarlinha(valorusuario,linha,matriz)==1) &&
         ...
         (matrizauxiliar[linha][coluna] == 0)) //se a posiÃ§Ã¢o estiver um valor randomico ele Ã© imutavel
       {
       ...
       }else if(tamanhovalor(valorusuario)==0)
       {
         ...
         system("cls");
       }else if((verificaposicao(linha,coluna,matriz)==0)&&(matrizauxiliar[linha][coluna] == 0)) //esse caso Ã© se a posiÃ§Ã£o jÃ¡ estiver ocupada !! se o valor digitado for != 0 ,a matrizauxiliar for ==0 entÃ¢o o susuario pode trocar o valor do local 
       {
         ...
         imprimirtabuleiro(matriz);
          if((opcao1 == 1) &&
            (verificarlinha(valorusuario,linha,matriz)==1) &&   //chama as funÃ§oes para verificar se o valor da troca Ã© valido.
            (verificarcoluna(valorusuario,coluna,matriz)==1)&&
            (verificabloco(valorusuario,linha,coluna,matriz)==1))
          {
            matriz[linha][coluna] = valorusuario;
       ...
       }else if(matrizauxiliar[linha][coluna] !=0) //nesse caso a posiÃ§Ã¢o Ã© imutavel
       {
